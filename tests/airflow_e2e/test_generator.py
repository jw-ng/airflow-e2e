...
def test_should_create_docker_compose_tests_yml_file_in_docker_base_folder():
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        docker_compose_tests_yml_file_path = Path(temp_dir) / "docker" / "docker-compose-tests.yml"
...
def test_should_setup_correct_dags_folder_in_docker_compose_tests_yml_file():
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        expected_docker_compose_tests_yml_file_path = Path(__file__).resolve().parent / "resources" / "expected_docker-compose-tests.yml"
